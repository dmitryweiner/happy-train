...
const closeTo = (a: number, b: number, epsilon = EPSILON): boolean => Math.abs(a - b) < epsilon;

const isBetween = (value: number, min: number, max: number, epsilon = EPSILON): boolean => value <= (max + epsilon) && value >= (min - epsilon);

function isClockwise(cellType: CellType, direction: number): boolean {
...
export function calculateTurnPosition(
  ...
  const getCenters = (cellType: CellType): { cx: number; cy: number } => {
    switch (cellType) {
      case CELL_TYPES.TURN_RIGHT_UP:
      case CELL_TYPES.SWITCH_RIGHT_UP_V:
      case CELL_TYPES.SWITCH_RIGHT_UP_H:
      ...
      default:
        throw new Error(`Not a turn cell: "${cellType}"`);
    }
  };
...
const SWITCH_CELLS: readonly string[] = [
...
export function isSwitchCell(cellType: string): boolean {
  return SWITCH_CELLS.includes(cellType);
}
...
export function isSemaphoreAtPosition(semaphores: readonly GridPoint[] | null | undefined, x: number, y: number): boolean {
  if (!semaphores) return false;
  return semaphores.some(semaphore => semaphore.x === x && semaphore.y === y);
}

const TURN_CELLS: readonly string[] = [
...
function isTurnCell(cellType: string): boolean {
  return TURN_CELLS.includes(cellType);
}

// Helper method to determine switch behavior based on approach direction
export function shouldTurnOnSwitch(switchType: CellType, direction: number, isStraight: SwitchState): boolean {
...
export function calculateStraightPosition(
  cellType: CellType,
  pixelX: number,
  pixelY: number,
  direction: number,
  ...
  let nextDirection: number = direction;
